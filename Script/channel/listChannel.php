...
while ($GroupeList = $req->fetch()) {
    ...
    if ($Account['id'] == $_SESSION['id']) {
        echo '<td><a href="manageChannel.php?Channel_id=' . $_GET['Channel_id'] . '&quitGroup=1&idGroupList=' . $GroupeList['id'] . '" title="Quiter le groupe">Quiter le groupe</a></td>   ';
    } elseif ($Groupe['creator_id'] == $_SESSION['id'] OR $GroupeList['super_user'] == 1) {
        $super_user = true;
        if ($Groupe['creator_id'] != $Account['id'] && $Account['id'] != $_SESSION['id']) {
            echo '<td><a href="manageChannel.php?Channel_id=' . $_GET['Channel_id'] . '&&ban=1&id=' . $Account['id'] . '" title="Banir"> Banir |</a>';
            if ($Account['id'] == $_GET['id'] && $_GET['ban'] == 1 && $_GET['Channel_id'] && !$_GET['up']) {
                echo '
            ...
            if ($GroupeList['super_user'] == 0) {
                echo '<a href="manageChannel.php?Channel_id=' . $_GET['Channel_id'] . '&up=1&id=' . $Account['id'] . '" title="Promouvoir"> Promouvoir</a></td>';
            } elseif ($GroupeList['super_user'] == 1) {
                echo '<a href="manageChannel.php?Channel_id=' . $_GET['Channel_id'] . '&up=0&id=' . $Account['id'] . '" title="Dépromouvoir"> Enlevez les drois</a></td>';
            }
        }
