...
if ($_GET['id_Channel'] && $_GET['id_Channel'] != null) {
    $SmessagesGroupe = $bdd->prepare('SELECT * FROM Messages_Channel WHERE id_channel = ? ORDER BY id');// RECHERCHE LES MSG AVEC LE CONTACT SELECTIONE
    $SmessagesGroupe->execute(array($_GET['id_Channel']));

    $allowList = $bdd->prepare('SELECT * FROM Channel_Contact_List WHERE channel_id = ? AND account_id = ?');
    $allowList->execute(array($_GET['id_Channel'], $_SESSION['id']));
    ...
    while ($message = $SmessagesGroupe->fetch()) {
        ...
        if ($message) {
            if ($message['id_sent'] != $_SESSION['id']) {
                ...
                <div class="row">
                    ...
                    <?php if ($GroupeList['super_user'] == 1) // MOderattor
                    {
                    echo '<a href="Script/channel/delMessageChannel.php?id=' .  $message['id'] . '&idContact=' . $_GET['id_Channel'] . '" >-</a>';
                    } ?>
                ...
                <div class="row">
                    ...
                    <?php echo '<a href="Script/channel/delMessageChannel.php?id=' .  $message['id'] . '&idContact=' . $_GET['id_Channel'] . '" >-</a>'; ?>
                </div>
    ...
    if ($GroupeList) {
        ...
        if ($GroupeList['allow'] == 1) {
           ...
           ?>
            ...
            <form method="post" action="Script/uploadFile.php?id_Channel=<?php echo $_GET['id_Channel']; ?>" enctype="multipart/form-data">
