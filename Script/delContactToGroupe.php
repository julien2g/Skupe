...
$selectSuperUser->execute(array($_GET['id_groupe'], $_SESSION['id']));
$GroupeList = $selectSuperUser->fetch();

if ($GroupeList['super_user'] == 1) $super_user = true;

$selectCreatorGroupe = $bdd->prepare('SELECT creator_id FROM Groupe WHERE id = ?'); // permet d'identifier le createur pour lui donner les pleins droit
$selectCreatorGroupe->execute(array($_GET['id_groupe']));
$creatorId = $selectCreatorGroupe->fetch();
if ($creatorId['creator_id'] == $_SESSION['id']) $creator = true;


$selectGroupList = $bdd->prepare('SELECT * FROM Groupe_Contact_List WHERE groupe_id = ?'); // cherches ts les membres
$selectGroupList->execute(array($_GET['id_groupe']));
...
foreach ($listFriend as $f) {
    ...
    if ($_POST[$f]) {
        $delGoupeList = $bdd->exec('DELETE FROM Groupe_Contact_List WHERE groupe_id = ' . $_GET['id_groupe'] . ' AND account_id = ' . $_POST[$f]);
